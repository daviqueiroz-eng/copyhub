...
export const RevisaoInteligenrePanel = ({
  ...
  return (
    <div className="fixed bottom-4 left-0 right-0 z-30 pointer-events-none px-4">
      <div
        className={cn(
          "pointer-events-auto mx-auto w-full max-w-[1100px] rounded-2xl border border-border/60 bg-background/95 backdrop-blur-md shadow-[0_8px_30px_-12px_hsl(var(--foreground)/0.18)] overflow-hidden transition-all",
          open ? "max-h-[260px]" : "max-h-12"
        )}
      >
        {/* Barra compacta — sempre visível */}
        <div className="flex items-center justify-between gap-4 pl-5 pr-3 h-11">
            ...
            {CATEGORIAS.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex items-center gap-2 text-xs whitespace-nowrap transition-all hover:opacity-100 group",
                    isActive ? "opacity-100" : "opacity-90"
                  )}
                >
                  <span className={cn("h-2 w-2 rounded-full shrink-0 ring-2 ring-background", cat.dot)} />
                  <span className="font-semibold tabular-nums text-foreground text-[13px]">
                    {counts[cat.tipo]}
                  </span>
                  <span
                    className={cn(
                      "text-muted-foreground group-hover:text-foreground transition-colors text-[11.5px]",
          ...
          <div className="flex items-center gap-0.5 shrink-0 border-l border-border/50 pl-2 ml-1">
            {isAnalyzing && (
        ...
        {/* Painel expandido */}
        {open && (
          <div className="border-t grid grid-cols-1 md:grid-cols-[260px_1fr] max-h-[220px]">
            {/* Lista da categoria */}
            <div className="border-b md:border-b-0 md:border-r flex flex-col min-h-0">
              ...
              <ScrollArea className="flex-1 max-h-[185px]">
